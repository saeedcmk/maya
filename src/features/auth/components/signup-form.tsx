...
function SignUpForm() {
	...
	const formSchema = useMemo(
		() =>
			z.object({
				nickname: z.string().nonempty(t("errors.required")),
	...
	const [loginLink, setLoginLink] = useState<string>("/auth/signup");

	useEffect(() => {
		setLoginLink(`/auth/login${document?.location.search}`);
	}, []);
						...
						>
							<FormField
								...
								render={({ field }) => (
									<FormItem className="max-xs:max-w-80 mx-auto">
										<FormLabel>
											{t("signup-form.fields.nickname.title")}
										</FormLabel>
										<FormControl>
											<Input {...field} />
										</FormControl>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem className="max-xs:max-w-80 mx-auto">
										<FormLabel>
											{t("signup-form.fields.username.title")}
										</FormLabel>
										<FormControl>
											<Input {...field} />
										</FormControl>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem className="max-xs:max-w-80 mx-auto">
										<FormLabel>
											{t("signup-form.fields.password.title")}
										</FormLabel>
										<FormControl>
							...
							<div className="space-y-2">
								...
								>
									<Spinner
										...
										size="sm"
									>
										{t("signup-form.actions.signup")}
									</Spinner>
								</Button>
							</div>

							<p className="text-center">
								{t("signup-form.actions.has_account.description")}{" "}
								<Link href={loginLink} prefetch={false}>
									{t("signup-form.actions.has_account.link")}
								</Link>
							</p>
						</div>
					</form>
